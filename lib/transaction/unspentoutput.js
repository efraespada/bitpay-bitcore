...
function UnspentOutput(data) {
  ...
  if (data.txid !== undefined) {
    txId = data.txid;
  } else if (data.txId !== undefined) {
    txId = data.txId;
  } else if (data.mintTxid !== undefined) {
    txId = data.mintTxid;
  }

  if (!txId || !/^[0-9a-fA-F]+$/.test(txId) || txId.length > 64) {
    throw new Error(`Invalid TXID in object ${txId}`, data);
  }

  var outputIndex = 0;
  if (data.vout !== undefined) {
    outputIndex = data.vout;
  } else if (data.outputIndex !== undefined) {
    outputIndex = data.outputIndex;
  } else if (data.mintIndex !== undefined) {
  ...
  $.checkArgument(
      !_.isUndefined(data.amount) || !_.isUndefined(data.satoshis) || !_.isUndefined(data.value),
      'Must provide an amount for the output'
  );
  let amount = 0;
  if (data.amount !== undefined) {
    amount = new Unit.fromBTC(data.amount).toSatoshis();
